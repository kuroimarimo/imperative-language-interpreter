...
int rule_cout()
{
     ...
     *type = DOUBLE_NUMBER;
     ...
     addVar(concat("#", data), getTableStackElem(localSTstack, 0), VAR_STRING);
     coordinates = varToFrame(concat("#", data));
     *type = STRING;
     break;
...
int rule_funcCall(hashElem * assignee)	//id ( <call_list> ;
{
    ...
    //printf("Velkost zakladneho ramca: %d\n", *findElem(globalST, funcCall.key)->data.baseFrameSize);
...
void processParam(int * paramIndex, hashElem * funcCall, int builtIn)
{
    ...
    //tVarCoordinates * coordinates = constToVar(token.type, data);
